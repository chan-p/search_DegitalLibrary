...
class category:
    ...
    def add_id(self):
        da = dt.now()
        date = '{}-{}-{}T{}:{}:{}'.format(da.year, da.month,
                                          da.day, da.hour, da.minute, da.second)
        ...
        self.__es.index(index="search_degital_library", doc_type="categories", id=str(
            id_), body={"name": self.name, "created": datel, "modified": datel})

    def get_book_ids(self):
        ids = []
        books = self.__table_books_categories.find(
            category_id=self.get_id(True))
        for book in books:
