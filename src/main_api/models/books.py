...
class book:
    ...
    def get_category(self):
        books = []
        categories = [record['_source']['category_id'] for record in self.__es.search(index="search_degital_library", doc_type="books_categories", body={
                                                                                      "query": {"match": {'book_id': self.id}}, "size": 200, "sort": [{"name": {"order": "desc"}}]})['hits']['hits']]
    ...
    def get_categories(self):
        ...
        for record in self.__ES.return_all_records("books_categories"):
            ...
            books[record['_source']['book_id']].append(
                categories[record['_source']['category_id']])
        return books
    ...
    def get_ids_date(self, date):
        ...
        print("SELECT * FROM books where created between " +
              str(date) + " and " + str(next_day))
        result = self.__db.query(
            "SELECT * FROM books where created between '" + str(date) + "' and '" + str(next_day) + "'")
    ...
    def search_ids(self, word):
        title_list = []
        result = self.__db.query(
            "SELECT id FROM books where name like '%" + word + "%'")
        for record in result:
            title_list.append(record['id'])
        return title_list

    def add_title(self):
        data = dict(name=self.title, created=self.__return_date(),
                    modified=self.__return_date())
        self.__table_books.insert(data)
        date = self.__table_books.find_one(id=self.get_id(True))["created"]
        self.__ES.add_record("books", self.get_id(
            True), {"name": self.title, "created": date, "modified": date})
    ...
    def add_related_category(self, category_id):
        ...
        id_ = self.__table_books_categories.find_one(
            book_id=self.id, category_id=category_id)['id']
        self.__ES.add_record("books_categories", id_, {
                             "book_id": self.id, "category_id": category_id})
